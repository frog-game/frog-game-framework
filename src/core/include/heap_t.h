...
#define HEAP_EXPORT(declaration) __UNUSED static declaration
...
HEAP_EXPORT(void heap_remove(struct heap* heap, struct heap_node* node, heap_compare_fn less_than))
{
    ...
    *max  = NULL;
    ...
    for (;;) {
        smallest = child;
        if (child->left != NULL && less_than(child->left, smallest)) smallest = child->left;
        if (child->right != NULL && less_than(child->right, smallest)) smallest = child->right;
