...
{
    lua_pushlightuserdata(L, tpackStreamCreate());
    return 1;
}

typedef struct ltpackFrame_s
{
...
static int32_t lnew(struct lua_State* L)
{
    ...
    ltpackFrame_tt* pFrame = lua_newuserdatauv(L, sizeof(ltpackFrame_tt), 0);
...
{
    ltpackFrame_tt* pFrame = lua_touserdata(L, 1);
    ...
    uint8_t      szToken[4];
    tpackHead_tt head;
    int32_t      r = tpack_decodeHead(&byteQueue, &head);
    if (r == 1) {
        if (head.uiOffset > 4) {
...
int32_t luaopen_lruntime_tpack(struct lua_State* L)
{
#ifdef luaL_checkversion
    luaL_checkversion(L);
#endif
    luaL_Reg lualib_tpackFuncs[] = {{"codecHandle", lhandle},
                                    {"new", lnew},
                                    {"write", lwrite},
                                    {"read", lread},
                                    {"reset", lreset},
                                    ...
                                    {"event", levent},
                                    {NULL, NULL}};
    luaL_newlib(L, lualib_tpackFuncs);
    return 1;
}
