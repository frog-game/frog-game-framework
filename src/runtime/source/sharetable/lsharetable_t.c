...
static void mark_shared(lua_State* L)
{
    ...
    while (lua_next(L, -2) != 0) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            switch (type) {
            case LUA_TTABLE: mark_shared(L); break;
            case LUA_TNUMBER:
            case LUA_TBOOLEAN:
            ...
            case LUA_TFUNCTION:
                ...
                break;
            case LUA_TSTRING: lua_sharestring(L, idx); break;
            default: luaL_error(L, "Invalid type [%s]", lua_typename(L, type)); break;
            }
        }
        lua_pop(L, 1);
    }
...
static int lis_sharedtable(lua_State* L)
{
    int b = 0;
    if (lua_type(L, 1) == LUA_TTABLE) {
        Table* t = (Table*)lua_topointer(L, 1);
...
static int clone_table(lua_State* L)
{
...
static int lco_stackvalues(lua_State* L)
{
...
static int close_state(lua_State* L)
{
...
static int box_state(lua_State* L, lua_State* mL)
{
...
static int load_matrixfile(lua_State* L)
{
    luaL_openlibs(L);
...
static int matrix_from_file(lua_State* L)
{
    ...
    if (top > 1) {
        ...
        for (i = 2; i <= top; i++) {
            switch (lua_type(L, i)) {
            case LUA_TBOOLEAN: lua_pushboolean(mL, lua_toboolean(L, i)); break;
            case LUA_TNUMBER:
                ...
                break;
            case LUA_TLIGHTUSERDATA: lua_pushlightuserdata(mL, lua_touserdata(L, i)); break;
            case LUA_TFUNCTION:
            ...
            default: return luaL_argerror(L, i, "Type invalid");
            }
        }
    }
...
LUAMOD_API int luaopen_lruntime_sharetable(lua_State* L)
{
    luaL_checkversion(L);
    luaL_Reg l[] = {
        {"clone", clone_table},
